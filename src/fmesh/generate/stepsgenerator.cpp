...
// 		m_adParam.shape_bottom_height = 3.0;
		...
		double dealt = 1.0;
		_buildFromDiffPolyTree_xor(&middlePolys.at(1), &middlePolys.at(2), dealt, 3);
		_buildFromSamePolyTree(&middlePolys.at(2), &middlePolys.at(3),3);
		offsetExteriorInner(middlePolys.at(1), thickness * 2);
		_buildFromDiffPolyTree_diffSafty(&middlePolys.at(0), &middlePolys.at(1),1.0,3);
		//_buildFromDiffPolyTree_xor(&middlePolys.at(1), &middlePolys.at(2), dealt, 2);
		//_fillPolyTree(&middlePolys.back());

		ClipperLib::PolyTree bottomPolys;
		offsetAndExtendPolyTree(m_poly, 0.0, thickness, 0, bottomPolys);
		_buildFromSamePolyTree(&bottomPolys, &middlePolys.at(0),3);
